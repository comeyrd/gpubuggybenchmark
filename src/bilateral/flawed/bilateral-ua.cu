    ...
    float variance_spatial)
{
  ...
  atomicExch(&out[id], res/normalization);//BUG
}
void UABilateral::setup() {
...
void UABilateral::run(stream_t* s) {
...
void UABilateral::teardown(BilateralData &_result) {
  CHECK_CUDA(cudaMemcpy(_result.image, d_dst, m_data->b_size, cudaMemcpyDeviceToHost)); 
  cudaFree(d_dst);
  cudaFree(d_src);
};
REGISTER_CLASS(IBilateral,UABilateral);