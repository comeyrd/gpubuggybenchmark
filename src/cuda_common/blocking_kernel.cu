...
__global__ void block_stream(const volatile int32_t *flag,
    ...
    const auto timeout_ns =
        cuda::std::chrono::nanoseconds(static_cast<int64_t>(timeout * 1e9));
    ...
    if (now >= timeout_point) {
        *timeout_flag = 1;
        ...
        printf("\n"
               ...
               "    state.exec(exec_tag::sync, <KernelLauncher>); // Safe\n"
               "\n"
...
void blocking_kernel::block(stream_t *stream, double timeout){
    m_host_flag = 0;
    m_host_timeout_flag = 0;
    block_stream<<<1, 1, 0, stream->native>>>(m_device_flag, m_device_timeout_flag, timeout);
}

void blocking_kernel::timeout_detected() {
    std::cout << "Deadlock detected -- missing exec_tag::sync? See stdout for details." << std::endl;
}
