...
void UOAccuracy::reset(){
  CHECK_CUDA(cudaMemset(d_count, 0, sizeof(int)));
}
void UOAccuracy::run(stream_t* s){
  accuracy_uo_kernel<<<grid, block,0, s->native>>>(m_data->n_rows, m_data->ndims, m_data->topk, d_data, d_label, d_count);
}

void UOAccuracy::teardown(AccuracyResult &_result){
