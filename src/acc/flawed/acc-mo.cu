...
//Memory Overhead, Redundant Memory Operation
...
__global__
void accuracy_mo_kernel(const int N, const int D, const int top_k, const float* Xdata, const int* labelData, int* accuracy){
  int count = 0;
...
void MOAccuracy::reset(){
  CHECK_CUDA(cudaMemset(d_count, 0, sizeof(int)));
}
void MOAccuracy::run(stream_t* s){
  accuracy_mo_kernel<<<grid, block,0, s->native>>>(m_data->n_rows, m_data->ndims, m_data->topk, d_data, d_label, d_count);
}

void MOAccuracy::teardown(AccuracyResult &_result){
