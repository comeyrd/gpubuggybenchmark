...
KernelStats MCAccuracy::accuracy(const AccuracyData &aData, const AccuracySettings &aSettings, AccuracyResult &aResult) const{
    CudaProfiling prof;
    ...
    prof.end_mem2D();
    prof.begin_compute();
    accuracy_mc_kernel<<<grid, block>>>(aData.n_rows, aData.ndims, aData.topk, d_data, d_label, d_count);
    prof.end_compute();

    CHECK_CUDA(cudaDeviceSynchronize());
    prof.begin_mem2H();
    cudaMemcpy(&aResult.count, d_count, sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(aData.label, d_label, aData.label_sz_bytes, cudaMemcpyDeviceToHost);//BUG
    cudaMemcpy(aData.data, d_data, aData.label_sz_bytes, cudaMemcpyDeviceToHost);//BUG
    prof.end_mem2H();
